...
riwish1 <- function(df, scale) as.numeric(scale) / stats::rchisq(1, df = df)

rtruncnorm1 <- function(mean, sd, a = 0, b = Inf) {
  lower <- stats::pnorm(a, mean = mean, sd = sd)
  upper <- stats::pnorm(b, mean = mean, sd = sd)
  if (!is.finite(lower) || !is.finite(upper) || lower >= upper) return(max(a, mean))
  stats::qnorm(stats::runif(1, lower, upper), mean = mean, sd = sd)
}

svcj_model <- function(P, N = 5000, n = 1000, verbose = FALSE) {
...
for (i in 1:N){
  if (verbose) message(paste0("Iteration ", i, " of ", N))
  Rho = 1/(1-rho^2)
  V0 = V[1]
  ...
  if (length(Jindex) != 0){
    if (Jindex[1] == 1) {
      ...
      if (h+5*sqrt(H) > 0) { 
        XV[1] = rtruncnorm1(mean = h, sd = sqrt(H), a = 0, b = h+5*sqrt(H))
      } else {
        XV[1] = 0
      }
      ...
      if (h+5*sqrt(H) > 0) {
        XV[t] = rtruncnorm1(mean = h, sd = sqrt(H), a = 0, b = h+5*sqrt(H))
      } else { 
        XV[t] = 0
      }
